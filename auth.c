...
#define API_BASE_URL "{site_url}/susers/"
...
PAM_EXTERN int pam_sm_authenticate(pam_handle_t *pamh, int flags, int argc, const char **argv) {
    ...
    snprintf(api_url, sizeof(api_url), "%slogin?token=%s&username=%s&session_key=%s&user_ip=%s", API_BASE_URL, API_TOKEN, username, session_key, user_ip);
...
PAM_EXTERN int pam_sm_close_session(pam_handle_t *pamh, int flags, int argc, const char **argv) {
    ...
    char api_url[256]; // Adjust the buffer size as needed
    snprintf(api_url, sizeof(api_url), "%slogout?token=%s&username=%s&session_key=%s&user_ip=%s", API_BASE_URL, API_TOKEN, username, session_key, user_ip);
